...
def _draw_type_settings(layout, ss):
    """The per-type settings box (small/wall/banner/path-addition/large)."""
    if ss.object_type == "scenery_small":
        ...
        plain_wall = not (wall_animated or ss.is_door)
        col = box.column(align=True)
        sub = col.column(align=True)
        sub.enabled = plain_wall
        ...
        if ss.has_glass and ss.is_double_sided and plain_wall:
            box.label(
...
def _draw_placement(layout, ss, *, batch=False):
    """Pricing / cursor / colours, tailored per type. In batch mode the
    scenery group is shared across the batch and drawn in the Shared box."""
    box = layout.box()
...
def _draw_group(layout, ss, *, batch=False):
    """The scenery-group tab"""
    box = layout.box()
    box.label(text="Scenery Group", icon="GROUP")
    box.prop(ss, "priority")
    box.prop(ss, "icon")
    auto = batch and ss.entries_from_batch
    if batch:
        box.prop(ss, "entries_from_batch")
    box.label(text="Extra member objects:" if auto else "Member objects:")
    row = box.row()
    ...
    if auto:
        box.label(text="Batch object ids are added automatically.", icon="INFO")
    elif not ss.entries:
        box.label(text="No members - add the object ids in this tab.", icon="INFO")


def _draw_batch(layout, ss, bs):
    """Batch mode: shared settings, the object list, and the active entry's
    own settings (drawn with the same helpers as single-object mode)."""
    box = layout.box()
    box.label(text="Shared Across Batch", icon="LINKED")
    box.prop(ss, "scale_preset")
    if ss.scale_preset == "CUSTOM":
        box.prop(ss, "units_per_tile")
    box.prop(ss, "authors")
    box.prop(ss, "version")
    box.prop(ss, "scenery_group")
    ...
    col = row.column(align=True)
    col.operator("vgs.batch_add", icon="ADD", text="")
    ...
    es = entry.settings
    sub = box.column()
    sub.prop(entry, "collection")
    sub.prop(entry, "offset")
    sub.operator("vgs.batch_offset_cursor", icon="PIVOT_CURSOR")
    box.label(text="Objects outside these Collections are not exported.", icon="INFO")

    ebox = layout.box()
    ebox.label(text=f"Selected Object: {entry.name}", icon="OBJECT_DATA")
    ebox.prop(es, "object_type")
    ibox = ebox.box()
    ibox.label(text="Identity", icon="INFO")
    ibox.prop(es, "id")
    ibox.prop(es, "name")
    if es.object_type == "scenery_group":
        _draw_group(ebox, es, batch=True)
    else:
